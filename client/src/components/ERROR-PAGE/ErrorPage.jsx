...
const ErrorPage = ({ msg, blogError }) => {
  ...
  useEffect(() => {
    ...
    switch (error || blogError) {
      ...
      case msg?.includes("There is no such a blog"):
        setMessage(
          "Sorry the Blog you are looking for is deleted by the owner"
        );
        timer = setTimeout(() => {
      ...
      case msg?.includes(
        "The blog you are looking for has been removed or deleted"
      ):
      ...
      case msg?.includes(
        "E11000 duplicate key error collection: blogAPI.categories"
      ):
  ...
  return (
    <section>
      <div >
        <h3>{message}</h3>
        <h5>You will be navigated to {path} page automatically in 3 seconds</h5>
      </div>
    </section>
  );
};
